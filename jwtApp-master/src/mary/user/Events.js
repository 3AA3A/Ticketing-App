    ...
    <>
      <Carousel>
        <Carousel.Item interval={5000}>
          ...
          />
          <Carousel.Caption>
            <h1 className='display-4' style={{textAlign:"left"}}>Music Festivals</h1>
            <p className='lead' style={{textAlign:"left", color:"lightgray"}}>View all ongoing events here!</p>
          </Carousel.Caption>
        </Carousel.Item>
        <Carousel.Item interval={5000}>
          <img
      ...
      <div className='col-md-9'>
        { events ?
          ...
          { genre > 0 && venue > 0 ?
                <CardDeck>
                {
                  filterBy(events,genre,venue).map(event => <Event key={event.id} event={event} />)
                }
                </CardDeck>
                : genre > 0 ? 
                <CardDeck>
                  {
                    filterBy(events,genre,0).map(event => <Event key={event.id} event={event} />)
                  }
                </CardDeck>
                : venue > 0 ?
                <CardDeck>
                {
                  filterBy(events,0,venue).map(event => <Event key={event.id} event={event} />)
                }
                </CardDeck> 
                : 
                <CardDeck>
